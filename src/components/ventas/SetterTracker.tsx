...
import {
  ...
  CheckCircle2, XCircle, Clock, AlertTriangle, ShoppingCart, Package
} from 'lucide-react';
...
export const SetterTracker = ({ clientId, hasAdAccount, onConvertToSale }: SetterTrackerProps) => {
  ...
  const handleStatusChange = async (appointment: SetterAppointment, newStatus: AppointmentStatus) => {
    if (newStatus === 'sold') {
      // Use convert to sale flow
      if (onConvertToSale) {
        onConvertToSale(appointment);
      }
      return;
    }
    try {
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {appointments.map(apt => {
                ...
                const canConvertToSale = apt.status !== 'sold' && apt.status !== 'cancelled';
                return (
                  <div
                    key={apt.id}
                  ...
                  >
                    ...
                    {canConvertToSale && onConvertToSale && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="h-8 text-[11px] px-3 flex-shrink-0 border-emerald-500/40 text-emerald-700 hover:bg-emerald-500/10 dark:text-emerald-400"
