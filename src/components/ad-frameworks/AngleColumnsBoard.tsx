...
export const AngleColumnsBoard = ({
  ...
  // If there's only one (placeholder) angle, swap the layout: columns become FORMATS,
  // and hooks remain stacked vertically inside each. This makes Format × Hook the natural view.
  const singleAngle = angles.length === 1;
...
const FormatColumn = ({
  angle, format, hooks, variantMap, renderVariant, onCreateMissing, syncPending,
}: {
  ...
  return (
    ...
    >
      <div className="px-3 py-2.5 border-b bg-card" style={{ borderLeft: `4px solid ${accent}` }}>
        <div className="flex items-center justify-between gap-2 mb-1.5">
          <h3 className="font-semibold text-sm truncate flex-1">{format.label}</h3>
