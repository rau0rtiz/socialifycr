...
export const SplashScreen = ({ onComplete, client, clientLogo }: SplashScreenProps) => {
  ...
  const prefetchAll = useCallback(async () => {
    ...
    const queries = [
      ...
      queryClient.prefetchQuery({ queryKey: ['setter-appointments', clientId, 'last_30d'], queryFn: async () => { const start = new Date(Date.now() - 30 * 86400000).toISOString(); const { data } = await supabase.from('setter_appointments').select('*').eq('client_id', clientId).gte('appointment_date', start).order('appointment_date', { ascending: false }); return data || []; }, staleTime: 2 * 60 * 1000 }),
      queryClient.prefetchQuery({ queryKey: ['client-setters', clientId], queryFn: async () => { const { data } = await supabase.from('client_setters').select('*').eq('client_id', clientId).order('name'); return data || []; }, staleTime: 5 * 60 * 1000 }),
      queryClient.prefetchQuery({ queryKey: ['client-products', clientId], queryFn: async () => { const { data } = await supabase.from('client_products').select('*').eq('client_id', clientId).order('name'); return data || []; }, staleTime: 5 * 60 * 1000 }),
      queryClient.prefetchQuery({ queryKey: ['sales-goals', clientId], queryFn: async () => { const { data } = await supabase.from('sales_goals').select('*').eq('client_id', clientId); return data || []; }, staleTime: 5 * 60 * 1000 }),
      queryClient.prefetchQuery({ queryKey: ['client-competitors', clientId], queryFn: async () => { const { data } = await supabase.from('client_competitors').select('*').eq('client_id', clientId); return data || []; }, staleTime: 5 * 60 * 1000 }),
    ];
