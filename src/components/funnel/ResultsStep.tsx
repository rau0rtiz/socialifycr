...
export const ResultsStep = ({ level, name, onCalendlyClick }: ResultsStepProps) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-2xl mx-auto text-center">
      <div>
        <p className="text-[#1a1a2e]/50 mb-2">Resultado para</p>
        <h2 className="text-3xl md:text-4xl font-black text-[#1a1a2e]" style={{ fontFamily: "'Poppins', sans-serif" }}>
          {name}
        </h2>
      </div>
        ...
        {levelData.map((l, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-2">
              ...
              <span className={`text-[10px] md:text-xs max-w-[60px] leading-tight ${isActive ? 'font-bold text-[#1a1a2e]' : 'text-[#1a1a2e]/40'}`}>
                {l.name}
              </span>
            </div>
          );
      ...
      <div className="p-6 md:p-8 text-left space-y-4 rounded-2xl border-2 bg-white" style={{ borderColor: current.color }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#1a1a2e]/50">{current.revenue}</p>
          </div>
        </div>
        <p className="text-[#1a1a2e]/70 leading-relaxed">{current.desc}</p>
      </div>
      ...
      {qualifiesForSession && (
        <div className="p-6 rounded-2xl bg-[#FF6B35]/5 border-2 border-[#FF6B35]/20 space-y-4">
          ...
          <Button
            size="lg"
            className="gap-2 bg-[#FF6B35] hover:bg-[#e55a2b] text-white font-bold px-8 rounded-xl"
