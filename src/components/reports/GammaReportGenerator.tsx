...
import { useCampaigns, useAdSets, useAds, CampaignInsights, AdSetInsights, AdInsights } from '@/hooks/use-ads-data';
...
// ── Campaign/AdSet/Ad selection tree component ──
...
const CampaignTree = ({
  ...
  // Fetch adsets for expanded campaigns
                          ...
                          <div className={cn(
                            ...
                            <div className="flex-1 min-w-0" onClick={() => onToggleAdSet(adset.id)}>
                              ...
                              <p className="text-[10px] text-muted-foreground">
                                Gasto: {currency} {adset.spend.toFixed(2)}
                              </p>
                            </div>
                          </div>
                          {/* Ads */}
...
export const GammaReportGenerator = ({ clientId, hasAdAccount }: GammaReportGeneratorProps) => {
  ...
  // Fetch campaigns for building report data (using selected date range)
  const customRange = dateRange.from && dateRange.to ? { from: dateRange.from, to: dateRange.to } : undefined;
  const { data: campaignsResult, isLoading: campaignsLoading } = useCampaigns(
    clientId,
  ...
  // Fetch sales filtered by date range
  ...
  // Helper to select/deselect all campaigns
  ...
  const buildDashboardData = useCallback(() => {
    ...
    if (selectedSources.includes('campaigns') && campaigns.length > 0) {
      // Filter campaigns by selection (if any selected, otherwise use all)
      ...
      data.campaigns = {
        ...
        selectedAdSetIds: selectedAdSetIds.size > 0 ? Array.from(selectedAdSetIds) : undefined,
        selectedAdIds: selectedAdIds.size > 0 ? Array.from(selectedAdIds) : undefined,
      };
    }
    ...
    if (selectedSources.includes('sales') && salesData.length > 0) {
      ...
      const byStatus = salesData.reduce((acc: Record<string, number>, s: any) => {
        acc[s.status] = (acc[s.status] || 0) + 1;
        return acc;
      }, {});

      // Include ad-attributed sales details
    ...
    return data;
  }, [selectedSources, campaigns, currency, salesData, socialPlatforms, selectedCampaignIds, selectedAdSetIds, selectedAdIds]);
  ...
  const selectionSummary = useMemo(() => {
    ...
    return parts;
  }, [selectedSources, selectedCampaignIds.size, campaigns.length, salesData.length, socialPlatforms.length]);
    ...
    <div className="space-y-6">
      ...
      {step === 'configure' && (
        <div className="grid gap-6 lg:grid-cols-2">
          {/* Left column: Date + Sources + Campaign Selection */}
          <div className="space-y-6">
            <Card>
              <CardHeader>
