                ...
                {filteredEmails.map((email: any) => (
                  <Card key={email.id} className="hover:bg-muted/30 transition-colors">
                    <CardContent className="p-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 text-sm text-muted-foreground">
                            ...
                            <Badge variant="outline" className="text-xs shrink-0">
                              {sourceLabels[email.source] || email.source}
                            </Badge>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="text-xs text-muted-foreground">
                            {format(new Date(email.created_at), "d MMM yyyy HH:mm", { locale: es })}
                          </span>
                          {email.html_content && (
                            <Button
                              variant="ghost"
                      ...
                      {email.error_message && (
                        <p className="text-xs text-red-500 mt-2 truncate">{email.error_message}</p>
                      )}
                    </CardContent>
                  </Card>
                ))}
        ...
        <Dialog open={!!previewEmail} onOpenChange={() => setPreviewEmail(null)}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="text-sm text-muted-foreground mb-2">
              Para: {previewEmail?.recipient_email}
              {' · '}
              {previewEmail?.created_at && format(new Date(previewEmail.created_at), "d MMM yyyy HH:mm", { locale: es })}
            </div>
            <div
