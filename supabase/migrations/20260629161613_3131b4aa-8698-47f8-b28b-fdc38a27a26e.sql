...
BEGIN
  ...
  -- Build deterministic seller list (setters + closers) for this client, ordered by user_id
  SELECT COALESCE(array_agg(ctm.user_id ORDER BY ctm.user_id), ARRAY[]::uuid[])
  INTO v_sellers
  ...
  -- Round-robin: count existing assigned leads for this client (any of those sellers) and pick next
