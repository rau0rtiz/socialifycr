...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch accounts using a short-lived token from FB JS SDK (no code exchange needed)
    if (action === 'fetch-accounts-token') {
      ...
      const { shortLivedToken, clientId } = body;

      if (!shortLivedToken || !clientId) {
        return new Response(JSON.stringify({ error: 'Missing shortLivedToken or clientId' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      console.log('Fetching accounts via FB SDK token for client:', clientId);
      ...
      // Get Instagram accounts linked to pages (parallel)
    ...
    // Fetch accounts without saving - requires authentication (legacy redirect flow)
