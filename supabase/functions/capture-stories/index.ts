...
import { aiFeatureEnabled } from "../_shared/ai-switch.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const ocrEnabled = await aiFeatureEnabled("ocr");
    const lovableApiKey = ocrEnabled ? Deno.env.get("LOVABLE_API_KEY") : undefined;
